...
import { Order, OrderStatus, TailorProductionStage } from '../models/Order';
import { validateStatusTransition, validateProductionStageTransition, getProductionStageProgress } from '../utils/orderStateMachine';
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const pricingSnapshot = calculateGarmentPrice(
      { basePrice: productSnapshot.basePrice, currency: 'ETB' } as any,
      customization
    );
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    if (tailorId) filter.tailorId = String(tailorId);
...
router.get('/:id', async (req: Request, res: Response) => {
  const { id } = req.params;
  try {
    const dbOrder = await Order.findOne({ $or: [{ orderId: id }, { _id: id }] });
    if (dbOrder) {
...
router.patch('/:id/status', async (req: Request, res: Response) => {
  ...
  try {
    orderObj = await Order.findOne({ $or: [{ orderId: id }, { _id: id }] });
    if (orderObj) isDb = true;
  } catch (err) {
    // memory fallback
  }
  ...
  const currentStatus: OrderStatus = orderObj.status as OrderStatus;
  const targetStatus: OrderStatus = newStatus as OrderStatus;

  // Validate state transition & actor permissions
  const validation = validateStatusTransition(currentStatus, targetStatus, actorRole);
  ...
  if (!orderObj.statusHistory) orderObj.statusHistory = [];
...
router.patch('/:id/production-stage', async (req: Request, res: Response) => {
  ...
  const validation = validateProductionStageTransition(currentStage, productionStage);

  if (!validation.isValid) {
    return res.status(400).json({ error: 'Invalid production stage transition', details: validation.reason });
  }

  const progressPercent = getProductionStageProgress(productionStage as TailorProductionStage);
  ...
  let syncStatus: OrderStatus = orderObj.status;
  if (productionStage === 'MEASUREMENT_VERIFIED') syncStatus = 'MEASUREMENT_VERIFICATION';
  else if (productionStage === 'CUTTING' || productionStage === 'SEWING' || productionStage === 'FINISHING') syncStatus = 'IN_PRODUCTION';
  else if (productionStage === 'QUALITY_CHECK') syncStatus = 'QUALITY_CHECK';
  else if (productionStage === 'READY') syncStatus = 'READY_FOR_PICKUP';
