...
export interface MeasurementDefinition {
  key: string;
  name: string;
  category: 'BASIC' | 'UPPER_BODY' | 'LOWER_BODY';
  standardUnit: string;
  minPlausible: number;
  maxPlausible: number;
  description: string;
...
export const MEASUREMENT_REGISTRY: Record<string, MeasurementDefinition> = {
  // --- BASIC ---
  height: {
    key: 'height',
    name: 'Height',
 ...
 */
export function verifyRequiredMeasurements(
  requiredKeys: string[],
  ...
  for (const key of requiredKeys) {
    ...
    const def = MEASUREMENT_REGISTRY[key];
