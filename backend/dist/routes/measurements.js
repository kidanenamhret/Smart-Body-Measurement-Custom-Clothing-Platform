...
router.get('/schema', (_req, res) => {
    return res.json({
        ...
        measurements: Object.values(measurements_1.MEASUREMENT_REGISTRY),
    });
});
// =========================================================================
...
router.post('/profiles', async (req, res) => {
    try {
        ...
        const existing = await BodyMeasurementProfile_1.BodyMeasurementProfile.find({
        ...
        if (measurements && typeof measurements === 'object') {
            for (const [key, item] of Object.entries(measurements)) {
                const rawVal = typeof item === 'object' ? item.value : Number(item);
                const rawUnit = typeof item === 'object' ? item.unit : 'cm';
                const rawSource = typeof item === 'object' ? item.source : source || 'MANUAL';
                const rawConfidence = typeof item === 'object' ? item.confidence : 1.0;
                const rawVerified = typeof item === 'object' ? !!item.verified : false;
                if (typeof rawVal === 'number' && !isNaN(rawVal)) {
                    const normalized = (0, measurements_1.normalizeMeasurementValue)(key, rawVal, rawUnit);
        ...
        if (isCurrent) {
            await BodyMeasurementProfile_1.BodyMeasurementProfile.updateMany({ customerId: customer._id, isCurrent: true }, { isCurrent: false });
        }
        const profile = new BodyMeasurementProfile_1.BodyMeasurementProfile({
            customerId: customer._id,
            profileName: profileName || 'Default Profile',
            version: nextVersion,
            isCurrent,
        ...
        await profile.save();
        if (isCurrent) {
            customer.defaultMeasurementProfileId = profile._id;
            await customer.save();
        }
...
router.get('/profiles', async (req, res) => {
    try {
        ...
        const profiles = await BodyMeasurementProfile_1.BodyMeasurementProfile.find({ customerId: customer._id })
...
router.get('/profiles/:id', async (req, res) => {
    try {
        ...
        const profile = await BodyMeasurementProfile_1.BodyMeasurementProfile.findOne({
            _id: req.params.id,
            customerId: customer._id,
        });
        if (!profile) {
...
router.put('/profiles/:id', async (req, res) => {
    try {
        ...
        const currentProfile = await BodyMeasurementProfile_1.BodyMeasurementProfile.findOne({
            _id: req.params.id,
            customerId: customer._id,
        });
        if (!currentProfile) {
        ...
        const latest = await BodyMeasurementProfile_1.BodyMeasurementProfile.findOne({
            customerId: customer._id,
        ...
        const newVersionProfile = new BodyMeasurementProfile_1.BodyMeasurementProfile({
            customerId: customer._id,
            profileName: targetName,
        ...
        customer.defaultMeasurementProfileId = newVersionProfile._id;
        await customer.save();
        return res.status(201).json({
 ...
 */
router.post('/profiles/:id/set-current', async (req, res) => {
    try {
        const customer = await Customer_1.Customer.findOne({ userId: req.user.sub });
        if (!customer) {
            return res.status(404).json({ message: 'Customer profile not found.' });
        ...
        await BodyMeasurementProfile_1.BodyMeasurementProfile.updateMany({ customerId: customer._id }, { isCurrent: false });
 ...
 */
router.post('/profiles/:id/snapshot', async (req, res) => {
    try {
        const customer = await Customer_1.Customer.findOne({ userId: req.user.sub });
        if (!customer) {
            return res.status(404).json({ message: 'Customer profile not found.' });
...
router.post('/sessions', async (req, res) => {
    try {
        ...
        const session = new MeasurementSession_1.MeasurementSession({
            customerId: customer._id,
...
router.get('/sessions', async (req, res) => {
    try {
        ...
        const sessions = await MeasurementSession_1.MeasurementSession.find({ customerId: customer._id }).sort({ createdAt: -1 });
...
router.get('/sessions/:id', async (req, res) => {
    try {
        ...
        const session = await MeasurementSession_1.MeasurementSession.findOne({
            _id: req.params.id,
            customerId: customer._id,
        });
        if (!session) {
 ...
 */
router.post('/sessions/:id/captures', async (req, res) => {
    try {
        const customer = await Customer_1.Customer.findOne({ userId: req.user.sub });
        if (!customer) {
            return res.status(404).json({ message: 'Customer profile not found.' });
        ...
        if (frontCapture || frontCaptureUrl)
            session.frontCaptureUrl = frontCapture || frontCaptureUrl;
        if (sideCapture || sideCaptureUrl)
            session.sideCaptureUrl = sideCapture || sideCaptureUrl;
        if (heightReference)
            session.heightReference = heightReference;
...
router.patch('/sessions/:id/verify', async (req, res) => {
    try {
        ...
        for (const [key, item] of Object.entries(finalMeasurements)) {
            const val = typeof item === 'object' ? item.value : Number(item);
            const unit = typeof item === 'object' ? item.unit : 'cm';
            const confidence = session.confidenceScores && session.confidenceScores[key] ? session.confidenceScores[key] : (session.captureType === 'MANUAL' ? 1.0 : 0.95);
        ...
        await BodyMeasurementProfile_1.BodyMeasurementProfile.updateMany({ customerId: customer._id, isCurrent: true }, { isCurrent: false });
        const newProfile = new BodyMeasurementProfile_1.BodyMeasurementProfile({
            customerId: customer._id,
            profileName: targetName,
            version: nextVersion,
            isCurrent: true,
        ...
        customer.defaultMeasurementProfileId = newProfile._id;
        await customer.save();
        return res.json({
 ...
 */
router.post('/sessions/:id/draft', async (req, res) => {
    try {
        const customer = await Customer_1.Customer.findOne({ userId: req.user.sub });
        if (!customer) {
            return res.status(404).json({ message: 'Customer profile not found.' });
