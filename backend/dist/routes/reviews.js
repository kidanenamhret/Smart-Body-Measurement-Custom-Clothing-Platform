...
router.post('/', async (req, res) => {
    try {
        ...
        const order = await Order_1.Order.findOne({ $or: [{ _id: orderId }, { orderId }] });
        if (!order) {
        ...
        const existingReview = await Review_1.Review.findOne({ orderId: order._id });
        if (existingReview) {
        ...
        if (targetTailorId) {
            const allTailorReviews = await Review_1.Review.find({ tailorId: targetTailorId });
            const avg = allTailorReviews.reduce((acc, curr) => acc + curr.rating, 0) / allTailorReviews.length;
            await Tailor_1.Tailor.findByIdAndUpdate(targetTailorId, {
                averageRating: Number(avg.toFixed(1)),
                reviewCount: allTailorReviews.length,
            });
        }
        // 5. Log Audit Event
        await (0, auditLogger_1.logAuditEvent)({ userId: String(customerId || order.customerId), role: 'CUSTOMER' }, 'CREATE_REVIEW', 'Review', String(review._id), { orderId: String(order.orderId), rating, tailorId: String(targetTailorId) });
...
router.get('/tailor/:tailorId', async (req, res) => {
    try {
        ...
        const reviews = await Review_1.Review.find({ tailorId }).sort({ createdAt: -1 });
