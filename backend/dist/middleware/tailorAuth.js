...
exports.requireTailorProfile = exports.requireTailorRole = void 0;
 ...
 */
const requireTailorProfile = async (req, res, next) => {
    if (!req.user || req.user.role !== 'TAILOR') {
        return res.status(403).json({ message: 'Access denied: Tailor role required.' });
    }
    try {
        const tailor = await Tailor_1.Tailor.findOne({ userId: req.user.sub });
        if (!tailor) {
...
exports.requireTailorProfile = requireTailorProfile;
